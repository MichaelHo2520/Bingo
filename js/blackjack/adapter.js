...
/* ============================================================================
   ...
   ── ★ DB 上的樣子 ────────────────────────────────────────────────────────
       ...
       game.bj = {
         ...
         over  這一局結算過了沒
         nx{pid:true} ★★ v1.95.0:結算過場「繼續」的票 —— **每一個還在房裡的座位
...
const MP = MPCore.create((function(){
  ...
  /* ==========================================================================
     ...
     ──────────────────────────────────────────────────────────────────────────
       ...
       ★ DB 上多一格 `bj.nx = { pid: true }` —— 這一局誰按過「繼續」。
  ...
  function advance(seq){
    ...
    txRound((g, b) => {
      ...
      // ★★★ v1.95.0:新的一局要把「繼續」那一格清掉(不清就等於下一局一開始就湊齊了)
      b.nx = {};
    });
  }
  ...
  return {
    ...
    /* ---------- 誤按離開的救援:同名接續時把這一場的籌碼搬過來(v1.97.0) ----------
       ...
       **這一場**的累計淨籌碼住在 game.bj.nets{pid} —— 兩個管道都在回答「他現在有多少錢」,
       ...
       ⚠ nx(過場「繼續」的票)**不必搬**:nextWait 只算「還在房裡 + 這一局有座位」的人,
    ...
    newGame(ids){
      ...
      return {
        ...
        bj: { seq: 0, rd: 0, k: 0, rot: rot, seats: seats,
              ...
              // ★★★ v1.95.0:這一局誰按過「繼續」(全部按完才推進;見四之二)
              nx: {} }
      };
    },
    applyGame(g, isPlaying){
      ...
      paint();
      maybeSettle();
      // ★★★ v1.95.0:全部人按過「繼續」→ 推進(不指定房主,誰先看到誰叫)
      maybeNext();
    },
