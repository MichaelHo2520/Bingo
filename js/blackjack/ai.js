...
const BJAI = (function(){
  ...
  /* ── ★ 三個難度的階梯是「決策資訊用得多不多」,而不是「敢不敢冒險」──────────
       easy 只看自己的點數,而且亂補
       mid  「照莊家的規矩打」—— 補到 17 就停。★ 完全不看莊家的明牌
       hard 完整基本策略(看明牌)+ 會數這一局翻開的牌
  ...
  const LEVEL_INFO = {
    easy: { emoji: "🙂", name: "新手",
            desc: "怕爆 —— 13 點就不敢補了,但有時手癢又多補一張;完全不看莊家的明牌" },
    mid:  { emoji: "🤔", name: "普通",
            desc: "照莊家的規矩打 —— 補到 17 就停;不看莊家的明牌" },
    hard: { emoji: "😈", name: "高手",
            desc: "完整基本策略(會看莊家的明牌)+ 會數這一局翻開的牌" }
  };
  ...
  /* ==========================================================================
     ...
     ── ★★ v1.85.0 拿掉加倍之後,"d" 剩下的意思是「能加倍就加倍,否則補」──────────
       BJ.legal().dbl 現在永遠 false → pick() 那一行會把 "d" 全部退成 "h",
       而**硬手的 9 / 10 / 11 退成補牌正好就是無加倍時的最佳解**,所以表原樣留著。
       ⚠⚠ 唯一的例外是 **A,7(軟 18)對 3~6**:標準表那一格是
          「**能加倍就加倍,否則停**」—— 退成 "h" 是錯的(會把好好的 18 點補爆)。
          所以那一格改成直接回 "s"(見 softMove)。
          ★ 這正是「把一個選項拿掉會靜靜地改壞另一個地方」的典型:
            退化的方向不是每一格都一樣,只有逐格看過才知道。
     ========================================================================== */
  ...
  function softMove(total, up){
    ...
    if(total === 18){                                   // A,7
      /* ⚠⚠ 標準表這一格是「能加倍就加倍,**否則停**」,而不是「否則補」——
         v1.85.0 拿掉加倍之後就直接寫 "s"(退成 "h" 會把 18 點補爆,見檔上那段)。
         ★ 於是 A,7 對 2~8 一律停、對 9 / 10 / A 才補(那正是無加倍的基本策略)。 */
      if(up <= 8) return "s";
      return "h";                                       // 9 / 10 / A
    }
  ...
  function pick(st, seat, level){
    ...
    /* ★ 加倍不合法時退成補牌。v1.85.0 之後 lg.dbl **永遠** false → 這一行等於
       「表裡的 d 一律當成 h」,而硬手 9/10/11 那三格退成補牌正好是最佳解
       (軟 18 那一格已經在 softMove 裡改成 "s",見那段註解)。
