...
const MP = MPCore.create((function(){
  ...
  function act(a){
    ...
    if(a === "draw"){
      /* ⚠ 與單機那支(solo.js 的 act)**逐字一樣**:手上有合法牌可出時預設不准抽,
         除非房規 freeDraw 開著。 */
  ...
  return {
    ...
    onRoomField(k, v){
      ...
      if(k === "unRules"){
        ...
        if(next.stack === rules.stack && next.unoCall === rules.unoCall &&
           next.playDrawn === rules.playDrawn && next.toLast === rules.toLast &&
           next.freeDraw === rules.freeDraw) return;
        rules = next;
    ...
    ownPrefs(){ return { turnSec: turnSec, unRules: UN.normRules(rules), big: BigMode.get() }; },
    usePrefs(o){
      ...
      // 大牌桌(v1.178.3):舊偏好沒有這欄 → 預設關。⚠ 這一刻畫面還在選單,
      // BigMode 的守衛會把它壓著不生效,等 showScreen("play"/"solo") 才套上去。
      BigMode.set(!!o.big);
    },
    ...
    api: {
      ...
      setRule(key, val){
        ...
        // ⚠ 五項都要比(見 onRoomField 那一份的說明)—— 漏一項 = 那一項按了沒反應
        if(next.stack === rules.stack && next.unoCall === rules.unoCall &&
           next.playDrawn === rules.playDrawn && next.toLast === rules.toLast &&
           next.freeDraw === rules.freeDraw) return;
