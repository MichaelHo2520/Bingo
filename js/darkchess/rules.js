...
/* ============================================================================
   ...
   ── ★★ 炮的三條(這是暗棋與象棋差最多的地方)────────────────────────────
     ...
     2. 沿直線隔**恰好一顆**子(任意顏色、明暗皆可)當炮架跳過去吃,
        **距離不限、不受階級限制**(炮可以吃將)。
     3. **炮可以打暗子,但受房規「連吃」(`chain`)管** —— `chain` 沒開,炮遇到暗子就當
        被擋住,那條線直接死掉(連目標都不算);開了才打得到。
        ★★ 但**翻開是自己人就只是翻開,兩顆都活**,不是吃掉 ——
          ...
          ⚠⚠ 「打不打得到暗子要看 `chain`」這條**查無出處**(查證過的來源都寫「無條件」,
            見一之表),是使用者在 2026-08-08 明確要求加的耦合 —— 不要照原始資料改回去。
    ...
    for(let di = 0; di < DIRS.length; di++){
      ...
      for(let k = 0; k < line.length; k++){
        ...
        /* 炮架之後的第一顆 = 目標。
           ...
           ★ 暗棋要看房規「連吃」(chain):沒開就當擋住,連目標都不算
             (使用者要求的耦合,查無出處,見檔頭第 3 條)。 */
        if(cell.up){
          if(sideOf(cell.p) !== mySide) out.push({ to: line[k], kind: "jump" });
        }else if(st.rules.chain){
          out.push({ to: line[k], kind: "jump" });
        }
        break;
      }
    }
