...
const DCB = (function(){
  ...
  /* ★★ 炮的**跳板**上蓋一個紅色 X(v2.5.2)—— 使用者原話:「選了的動作時,前面不能移動的
     是用藍色的虛線,我們有辦法改成畫個紅色 X 嗎?」。之前是一圈青色虛線(v2.3.7),
     而那一版的顧慮是「紅色會看起來像吃得到」—— **換成 X 之後那個顧慮消失了**:
     落點的語彙全部是「框 / 環」(方框 = 走、方框+圓環 = 吃),X 不屬於那一套,
     一眼就知道「這一格不是落點」。理由與色票寫在 styles.src.css 的 --dc-x。
     ⚠⚠ 為什麼是**多一個 DOM 節點**而不是再疊一層 CSS:這一格的 box-shadow(.dc-tgt/.dc-from)、
       ::before(.dc-lastto)、::after(落點的圓環)全都名花有主,而跳板那一格同時是其中
       任何一種都完全可能(v2.3.7 那條註解就是為此才選 outline 的)。X 需要**兩條線**,
       一層偽元素畫不出來(兩條各要自己的 transform)。
     ⚠ 走 SVG 不寫文字的 ✗:文字要處理字形變異(U+2717 / U+274C 那一票各家長得不一樣,
       還有 emoji 呈現的問題,見 CLAUDE.md 紅線 8),而且縮到 26px 會糊掉。
     ⚠ 同一條路徑畫兩次:粗的那條是**淺色描邊**(跳板可能是紅漆的字,紅蓋紅會看不清),
       細的那條才是紅 X。⚠ 不可以拿 box-shadow 或 filter 做描邊 ——
       兩條線交叉,單獨描邊會在交叉處留下一道淺色的橫痕。
     ⚠⚠⚠ 22~78 這個**很大**的跨距是為了「底下那顆還看得出是什麼棋」(使用者要求),
       而判準與直覺相反:**大而細比小而粗好認**。小 X 縮在正中央,壓的正好是字的重心
       (第一版 32~68 加 11 的線寬把 `車` / `卒` 壓成一團);拉到整顆棋子上、線收細之後,
       字是從四支手臂**之間**透出來的。透明度與線寬在 styles.src.css 的 --dc-x-op。
     ⚠ pointer-events 交給 CSS 關掉;點擊本來就走 closest("[data-sq]"),吃不掉。 */
  function xHTML(){
    return '<svg class="dc-x" viewBox="0 0 100 100" aria-hidden="true">' +
           '<path class="dc-xh" d="M22 22 78 78M78 22 22 78"/>' +
           '<path class="dc-xi" d="M22 22 78 78M78 22 22 78"/></svg>';
  }
  ...
  function paint(){
    ...
    /* ★ 選中炮的時候順便把**跳板**(炮架)標出來 —— 這是炮唯一「看不出來」的一段:
       ...
       ★ v2.5.2 起畫的是一個**紅色 X**(v2.3.7~v2.5.1 是一圈青色虛線),見 xHTML()。
  ...
  /* ---------- 記牌盤:還沒翻開的暗子裡,兩邊各還有哪幾種 ----------
     ...
     ⚠⚠⚠ 它**綁房規 foeCaps**,而這條耦合不是保守,是算出來的:
       ...
       而 foeCaps **關著**的時候(v2.5.2 起那是玩家自己選的、不再是預設),
       notes/19 那張房規表寫的就是「對手吃了什麼要自己記」。
