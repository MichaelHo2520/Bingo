...
/* ============================================================================
   ...
   ── ★ 這一頁刻意**沒有** JS 算牌寬 ────────────────────────────────────────
     ...
     二分回檢([notes/11](../../notes/遊戲/11-台灣麻將16張.md) 第六節),因為那副牌的張數
...
const SVB = (function(){
  ...
  /* ★★ 格子上疊了幾組**互不干擾**的記號(v2.5.0),每一組都只吃「我自己知道的東西」:
  ...
  function actsHTML(info){
    ...
    /* ★ 蓋牌的代價不只是罰分(v2.5.0)—— 蓋掉 ♠J,♠Q ♠K 從此也永遠出不了。
  ...
  function resultHTML(st, names, mySeat, foot, wins){
    ...
    /* ★ 蓋牌是這一頁全場唯一看不到的東西,所以結果卡上它們是**一列一列翻開**的
       (`--i` 推遲的動畫,v2.5.0)—— 一次全部跳出來等於把揭曉的那一刻偷走。
       遲到第四列也才 0.42s,不會讓人等。
  ...
  /* ==========================================================================
     八、動效(v2.5.0)—— ★★ 全部從「軌道的 diff」推,不在動作點插呼叫
     ──────────────────────────────────────────────────────────────────────────
