...
function showScreen(which){
  ...
  /* 大牌桌:每次換畫面都**無條件**重套一次(v1.178.5,見 ui-kit 的 BigMode)。
...
/* ---------- 啟動 ---------- */
buildSwatches();
/* 大牌桌(v1.178.5):⚠ 一定要排在 loadPrefs() **之前** —— 偏好會回頭叫 BigMode.set()。 */
BigMode.init({
  cls:"sv-big", btn:"sv-bigbtn", name:"大牌桌",
  live:()=>{ const el=$("svPlay"); return !!el && !el.classList.contains("hidden"); },
  save:savePrefs
});
