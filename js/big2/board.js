...
const B2B = (function(){
  ...
  /* ==========================================================================
     ...
     ──────────────────────────────────────────────────────────────────────────
       ...
       ★★ v1.83.0 換成**真牌桌的座位**。使用者的原話:
             「中間的顯示,我想把它改成像是四個人正常的座位分上下左右,
               自己永遠是在下方,然後其他一順序來顯示位置」
          → 自己**永遠**在下面,其他人照出牌順序繞一圈(下家右 · 對家上 · 上家左),
            每個人的最後一個動作就擺在他自己的位置上。
          舊版是「由舊到新的一列列清單」,它的三個缺點都是這一版解掉的:
     ...
     ========================================================================== */
  /* 座位在畫面上的位置 —— 自己永遠 "b"(下),其他人照**出牌順序**繞一圈。
     ★ 下家在**右邊**(照台灣牌桌的習慣:輪次是逆時針,下家坐你右手邊)。
     ⚠ 3 人局沒有對家(自己 + 左右);2 人局面對面(自己 + 上)——
       這兩張表不可以用「4 人那張砍一半」代替:3 人硬套 4 人表會把上家畫到「上」,
       左邊空一格、右邊也空一格,看起來像少了一個人。 */
  const SEAT_POS = { 2: ["b", "t"], 3: ["b", "r", "l"], 4: ["b", "r", "t", "l"] };

  /* 一個座位那一格。act = 他這一輪的最後一個動作(undefined = 還沒表態)。
     ⚠ 欄位順序是「名字 · 牌型 · 無敵」一列 + 「牌」一列(v1.79.1 的結論在座位版
       自然成立:牌一律從那一格的左緣開始,所以每一格的第一張牌都對齊)。 */
  function seatHTML(v, seat, pos, act, isTop){
    ...
    return '<div class="b2-tseat pos-' + pos + '">' +
             '<div class="' + cls + '">' +
               '<div class="b2-thd">' +
                 '<span class="b2-seat p' + seat + '"></span>' +
                 '<span class="b2-tnm">' + esc(nm) + '</span>' +
                 /* ⚠ 名字本身就叫「你」時(單機的 0 號位)不掛徽章 —— 「你 你」是純雜訊,
                    而「這一格是我」還有位置(永遠在下面)與底色在標。同 resultHTML 那條。 */
  ...
  /* ⚠ 這一整塊是 `.b2-trick`;裡面那一格格是 `.b2-tseats` / `.b2-tseat`。
  ...
  function trickHTML(v){
    ...
    /* 目前最大的那一手 = 最後一個「有出牌」的。
       ⚠ 用**座位**而不是索引:格子是照座位擺的。那個座位的最後一個動作一定就是
    ...
    /* ★★★ 格數 = **人數**,不是「動過的人數」(理由見上面那段:高度不准跳)。
       ⚠ 這是這一頁第二個寫進 JS 的數字(第一個是手牌的 --b2-slots),兩個都是
         **張數 / 人數這種資料**,不是量出來的尺寸 ——
         「這一頁不用 JS 算尺寸」那條沒有破。 */
    const pos = SEAT_POS[n] || SEAT_POS[4];
    let cells = "";
    for(let rel = 0; rel < n; rel++){
      const s = (me + rel) % n;
      cells += seatHTML(v, s, pos[rel], act[s], s === topSeat);
    }
    ...
    return '<div class="b2-trick' + (list.length ? "" : " empty") + '">' +
             ...
             '<div class="b2-tseats s' + n + '">' + cells +
               /* 桌子正中央:只有一個淡淡的圖示(牌是往中間打的,那裡不放資訊)。
                  ⚠ 一定要 🎴(U+1F3B4)—— 撲克牌 Unicode(U+1F0A0 那一段,含 🃏)
                    一個都不准進畫面,多數字型沒有會變豆腐方框(CLAUDE.md 的禁令)。 */
               '<div class="b2-tmid" aria-hidden="true">🎴</div>' +
             '</div>' +
           '</div>';
  }
  ...
  /* ==========================================================================
     ...
     ──────────────────────────────────────────────────────────────────────────
       ...
       ★ me / turn = 「我坐幾號」「現在輪到幾號」(v1.83.0 座位式牌桌要的兩個數字):
         me 決定桌面怎麼轉(自己永遠在下面)、turn 只用來畫那一格「思考中…」。
  ...
  function render(v){
    ...
    /* ✗ 「捲到最新那一手」那一段 v1.83.0 拿掉了 —— 座位式牌桌**沒有時間順序**,
       格子的位置由座位決定(自己永遠在下面)。舊版是「由舊到新的清單」才需要
       每次把 scrollTop 推到底。
       ⚠ 矮視窗放不下時 .b2-tseats 自己還是捲得動(overflow-y:auto),但**刻意不自動捲**:
         自動捲到底會把對家那一格推出畫面,而那一格與自己那一格一樣重要。 */
  }
