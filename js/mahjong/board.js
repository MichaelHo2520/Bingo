...
const MB = (function(){
  ...
  /* 花牌的小圖案。⚠ 刻意**不照真牌按植物配**(梅=紅梅花、冬=紅梅花……)——
     那會讓「梅」和「冬」長得幾乎一樣,但它們分屬不同配對群(梅蘭竹菊 / 春夏秋冬)、
     互相配不起來,玩家一定會以為是 bug。
     所以圖案按**群**走:春夏秋冬一律菊形、梅蘭竹菊一律梅形,群內靠字區分。
     這是「把規則畫出來」,不是提示 —— 哪兩張同群本來就是公開規則。 */
  ...
  function draw(code){
    ...
    // 花牌:群圖案(左上)+ 單字(右下)+ 細框。真牌的花牌也有一圈框,同時和字牌區隔開
    if(s==="h"||s==="p")
