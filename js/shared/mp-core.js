  ...
  function create(A){
    ...
    function askLeave(act){
      ...
      if(m)m.innerHTML=isHost
        ...
        /* ★★ 開了回座的遊戲,「離開就不算你的成績」已經是**錯的** —— 分數留著、位子也留著,
           從大廳按「🔄 回座」就回得去。但**不可以只講好消息**:對手那台的寬限只有 8 秒
           (明確按離開會寫 bye 記號,見四個寬限期),2 人局很可能回來時這一局已經作廢了。 */
        : (inGame?(REJOIN_MID
                   ?"這局還沒結束。你可以從大廳按「🔄 回座」回到這一場,但<b>其他人不一定會等你</b>。"
                   :"這局還沒結束,離開就<b>不算你的成績</b>。")
                :"確定要離開這個房間嗎?");
