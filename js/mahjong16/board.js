...
const M16B = (function(){
  ...
  let ord = null;
  /* ★★ **一定要是真的內部函式**,不可以只寫在 return 物件裡當方法簡寫:
       物件字面值的 `resetOrder(){…}` **不會**在這個 IIFE 的作用域建立同名綁定 ——
       而「一鍵理牌」那顆鈕的點擊處理器(mount 裡)是裸呼叫 `resetOrder()`。
     ⚠ v2.4.0~2.4.2 只有那個方法簡寫 → 按下去當場 `ReferenceError`,處理器停在第一行:
       順序沒清、沒重畫、鈕也不會消失,**症狀就是使用者回報的「按了沒反應」**。
       e2e 當時全綠(它只叫 API `M16B.resetOrder()`,從沒點過那顆鈕)—— 補在 e2e 的 ⑨ 節。
     ★ 同 `resetFit`:內部函式 + 下面用簡寫外露一次,兩條路共用同一份定義。 */
  function resetOrder(){ ord = null; }
  ...
  /* ★★ 地板的「暖身期」(v1.73.0,只有橫向啟用)——────────────────────────────
     ...
     ⚠ **刻意只在橫向啟用**(landscape() 為假時 warmUntil 歸零)。直向很可能有同一條
       ...
       → notes/遊戲/11-台灣麻將16張.md 第六節(「地板的暖身期 WARM_MS」那一段)。 */
